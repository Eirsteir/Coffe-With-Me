...
class CoffeeBreakServiceImplTest extends BaseUnitTest {
  ...
  private User addresseeAtSameUniversity;
  ...
  @Autowired private CoffeeBreakService coffeeBreakService;
  ...
  void setUp() {
    ...
    when(userRepository.findById(REQUESTER_ID)).thenReturn(Optional.ofNullable(currentUser));
    when(userRepository.findAllById(ADDRESSEE_IDS)).thenReturn(addressees);
    when(campusRepository.findById(CAMPUS_ID)).thenReturn(Optional.ofNullable(campus));
  ...
  @Test
  void
      testRegisterCoffeeBreakWhenValid_thenReturnRegisteredCoffeeBreakAddressedToFriendsAtSameUniversity() {
    CoffeeBreakRequest request = CoffeeBreakRequest.builder().campusId(campus.getId()).build();

    CoffeeBreakDetails savedCoffeeBreakDetails =
  ...
  void testRegisterCoffeeBreakWhenRequesterNotFound_thenThrowException() {
    ...
    CoffeeBreakRequest request = CoffeeBreakRequest.builder().build();
