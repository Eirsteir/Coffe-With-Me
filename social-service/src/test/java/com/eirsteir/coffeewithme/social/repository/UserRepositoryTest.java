...
class UserRepositoryTest {
  ...
  void setUp() {
    user =
        User.builder().id(ALEX_ID).nickname(USER_NAME_ALEX).email(EMAIL_ALEX).name("Alex").build();
    ...
    userJohn =
        entityManager.persistFlushFind(
            User.builder().id(JOHN_ID).name("john doe").email("john@doe.com").build());

    userTom =
        entityManager.persistFlushFind(
            User.builder().id(1006L).name("tom doe").email("tom@doe.com").build());

    userPercy =
        entityManager.persistFlushFind(
            User.builder().id(1007L).name("percy blackney").email("percy@blackney.com").build());
  }
