...
public class CoffeeBreakServiceImpl implements CoffeeBreakService {
  ...
  @Override
  public CoffeeBreakDetails registerCoffeeBreak(
      CoffeeBreakRequest coffeeBreakRequest, UserDetailsImpl currentUser) {
    ...
    ResultWithEvents<CoffeeBreakDetails> coffeeBreakWithEvents =
        CoffeeBreak.createCoffeeBreak(coffeeBreakDetails);
        ...
        .orElseThrow(
            () ->
                CWMException.getException(
                    EntityType.USER, ExceptionType.ENTITY_NOT_FOUND, userId.toString()));
  }

  private LocalTime getScheduledToFromNow(Long scheduledToInMinutes) {
    LocalTime now = LocalTime.now();
    if (scheduledToInMinutes == null) return now;
  ...
  private void publish(ResultWithEvents<CoffeeBreakDetails> coffeeBreakWithEvents) {
    ...
    domainEventPublisher.publish(
        CoffeeBreak.class, coffeeBreakWithEvents, coffeeBreakWithEvents.events);
  }
}
