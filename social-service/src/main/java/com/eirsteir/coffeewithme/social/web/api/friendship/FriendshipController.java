...
public class FriendshipController {
  ...
  Collection<FriendshipDto> getFriends(@AuthenticationPrincipal UserDetailsImpl principal) {
    ...
    if (friendships.isEmpty())
      throw new ResponseStatusException(
          ...
          "User with email - " + UserDetailsDto.getEmail() + " has no friends");
  ...
  FriendshipDto addFriend(
      @RequestParam("to_friend") Long toFriend,
  ...
  @PutMapping("/friends")
  FriendshipDto updateFriendship(
      @RequestBody @Valid FriendshipDto friendshipDto,
      @AuthenticationPrincipal UserDetailsImpl principal) {
  ...
  List<FriendshipDto> getFriendRequests(@AuthenticationPrincipal UserDetailsImpl principal) {
    ...
    List<FriendshipDto> friendRequests =
        friendshipService.findFriendshipsOf(userDto.getId(), FriendshipStatus.REQUESTED);

    if (friendRequests.isEmpty())
      throw new ResponseStatusException(
          HttpStatus.NO_CONTENT,
          "User with email - " + userDto.getEmail() + " has no friend requests");
  ...
  private void validateFriendshipDto(FriendshipDto friendshipDto, UserDetailsImpl principal) {
    ...
    if (addresseeId.equals(principalId)) return;
